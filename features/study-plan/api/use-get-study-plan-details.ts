...
export function useGetStudyPlanDetails(id: string | undefined) {
  const query = useQuery<StudyPlanDetails, Error>({
    queryKey: ["study-plan-details", id],
    enabled: !!id,
    queryFn: async () => {
      ...
      if (response.status !== 200) {
        ...
        const error = new Error(
          errorData.message || "Error al obtener los detalles del plan de estudio"
        );
