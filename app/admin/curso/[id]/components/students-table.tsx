...
import { ArrowUpDown, Search, Mail, Phone, User, UserMinus } from "lucide-react"
...
import type { EnrollmentData } from "@/features/course/api/use-get-course"
...
export function StudentsTable({ data, courseId }: StudentsTableProps) {
  ...
  const handleUnenroll = async (studentId: string, studentName: string) => {
    if (!confirm(`¿Estás seguro de desmatricular a ${studentName} de este curso?`)) {
      return
    }
    try {
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">DNI: {student.dni}</div>
            </div>
          </div>
        )
      },
          ...
          <div className="space-y-1">
            ...
            {student.studentProfile?.phone && (
              <div className="flex items-center gap-1.5 text-sm">
                ...
                <span className="text-muted-foreground">{student.studentProfile.phone}</span>
              </div>
            )}
        ...
        return (
          <div className="space-y-1">
            ...
            {profile?.guardianPhone && (
              <div className="flex items-center gap-1.5 text-sm">
                <Phone className="h-3.5 w-3.5 text-muted-foreground" />
                <span className="text-muted-foreground">{profile.guardianPhone}</span>
              </div>
            )}
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-muted-foreground mb-2">No hay estudiantes matriculados</p>
